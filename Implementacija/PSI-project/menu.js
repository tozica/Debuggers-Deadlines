...
//        }
    ...
    $('.panelProject').on('mouseenter', '.liElem', function (event) {
        ...
        tmpHtml.append($("<img>").attr("src", "http://localhost/PSI-project/Implementacija/img/moreOptions.png").css({
            "float": "right"
    ...
    $('.list').on('mouseleave', '.liElem', function (e) {
        ...
        if ($('.panelLabels img') != null)
            $('.panelLabels img').remove();
    })
    ...
    $(".labelForm").dblclick(function () {
        let form = $(this).parent();
        $('#flagDelete').val('false');
        form.submit();
    })
    ...
    $("#seeAllPremiums").click(function () {
        $("#formForSendToUser").html("");
        $("#formForSetPremium").html("");
        $("#formForRemovePremium").html("");
        $("#formForSendToAllUsers").html("");
        $("#formForPremiums").submit();
    });
    ...
    $("#InformAll").click(function () {
        let htmlString = "<table class='table'>"
                ...
                + "</tr>"
                + "<tr>"
                + "<td colspan='2'>Title: <input name='titleAdministrator' type='text' placeholder='Title: ' required='required'></td>"
                + "</tr>"
                + "<tr>"
    ...
    $("#InformUser").click(function () {
        let htmlString = "<table class='table'>"
                ...
                + "</td>"
                + "<td>"
                + "Username: <input name='usernameAdministrator' type='text' placeholder='To: ' required='required'>"
                + "</td>"
                + "</tr>"
                + "<tr>"
    ...
    $(".noticesAdministrator").click(function () {
        ...
        $("#noticeModalTitle").html("<h2 class='modal-title'>" + title + "</h2>");
        $("#noticeModalBody").html("<p>" + content + "</p>");
        $("#formForPremiumUser").html("");
        $("#formForPremiumUserDecline").html("");
        $("#formForPremiumUser").append("<input type='submit' class='btn btn-primary' value='Confirm'>");
        $("#formForPremiumUserDecline").append("<input type='submit' class='btn btn-outline-primary' value='Decline'>");
        ...
        $("#formForPremiumUser").append("<input type='hidden' name='idNotification' value=" + notificationId + ">");
        $("#formForPremiumUserDecline").append("<input type='hidden' name='idNotification' value=" + notificationId + ">");
    });
    ...
    $("#setPremium").click(function () {
        let htmlCode = "<table class='table'>"
                ...
                + "<tr>"
                + "<td>Username:</td>"
                + "<td><input type='text' placeholder='username' name='usernamePremium' required='required'></td>"
                + "</tr>"
                + "<tr>"
                ...
                + "<tr>"
                + "<td>Card namber:</td>"
                + "<td><input type='text' placeholder='card number' name='cardNumber' required='required'></td>"
                + "</tr>"
                + "<tr>"
                ...
                + "<tr>"
                + "<td>CVC:</td>"
                + "<td><input type='text' placeholder='cvc' name='cvc' required='required'></td>"
                + "</tr>"
                + "<tr>"
    ...
    $("#removePremium").click(function () {
        let htmlCode = "<table class='table'>"
                ...
                + "<tr>"
                + "<td>Username:</td>"
                + "<td><input type='text' placeholder='username' name='usernameRemovePremium' required='required'></td>"
                + "</tr>"
                + "<tr>"
                + "<td colspan='2'><input type='submit' value='Confirm' class='btn btn-primary'></td>"
                + "</tr>"
...
//    })
    ...
    $('.panelLabels').on('mouseenter', '.liElem', function (event) {
        event.preventDefault();
        console.log(event.currentTarget);
        event.stopPropagation();
        let tmpHtml = $("<img>").attr("src", "http://localhost/PSI-project/Implementacija/img/x.png").css({
            "float": "right"
        }).addClass("xIcon");
        $(this).append(tmpHtml);
    });
    $('.panelLabels').on('click', '.xIcon', function (event) {
        event.preventDefault();
        $('#flagDelete').val('true');
        $('#formLabels').submit();
    });
